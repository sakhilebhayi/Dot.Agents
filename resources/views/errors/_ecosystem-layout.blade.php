...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}" class="scroll-smooth">
    <head>
        ...
        <title>{{ $code }} — @yield('title') · Dot.Agents</title>
        ...
        @php
            ...
            $currentPlatformName = 'Dot.Agents';
        ...
        <style>
            ...
            * { box-sizing: border-box; }
            ...
            body { margin: 0; font-family: var(--font-body); background: var(--paper); color: #b9b3d6; min-height: 100dvh; display: flex; flex-direction: column; }
    ...
    <body>
        <header style="background: var(--chrome);">
            <nav style="max-width: 1400px; margin: 0 auto; padding: 14px 20px; display: flex; align-items: center; justify-content: space-between;">
                ...
                <div style="display: flex; align-items: center; gap: 12px;">
                    @auth
                        <a href="{{ route('dashboard') }}" class="press" style="padding: 10px 20px; background: var(--accent); color: #150f2e; font-family: var(--font-display); font-weight: 600; font-size: 14px; border-radius: 999px; text-decoration: none;">Dashboard</a>
                    @else
                        @if (Route::has('login'))
                    ...
                    <div style="border-top: 1px solid var(--line); padding-top: 28px; text-align: left;">
                        ...
                        <div style="display: flex; gap: 8px; overflow-x: auto; padding: 2px 2px 8px; scroll-snap-type: x proximity; -webkit-overflow-scrolling: touch;">
                            @foreach ($discover as $platform)
                                <a href="{{ $platform['url'] }}" class="press" style="flex: 0 0 auto; scroll-snap-align: start; display: flex; align-items: center; gap: 8px; padding: 7px 14px 7px 7px; background: #1d1640; border: 1px solid var(--line); border-radius: 999px; text-decoration: none; white-space: nowrap;">
        ...
        <footer style="background: var(--chrome-soft); padding: 24px 20px;">
            <div style="max-width: 1400px; margin: 0 auto; display: flex; flex-wrap: wrap; align-items: center; justify-content: center; gap: 16px;">
                <p class="font-mono" style="font-size: 12px; color: rgba(255,255,255,0.6); margin: 0;">&copy; {{ date('Y') }} Dot.Agents.</p>
