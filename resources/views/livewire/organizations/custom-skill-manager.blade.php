    ...
    <div class="bg-white dark:bg-gray-900 rounded-2xl border border-purple-200 dark:border-purple-800 p-6 space-y-4">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
                ...
                <input wire:model="name" type="text" class="w-full text-sm rounded-xl border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 text-gray-900 dark:text-white focus:ring-purple-600" placeholder="e.g. Inventory Lookup">
                @error('name') <p class="mt-1 text-xs text-red-500">{{ $message }}</p> @enderror
            </div>
            <div>
                <label class="block text-xs font-medium text-gray-700 dark:text-gray-300 mb-1.5">Timeout (seconds)</label>
                <input wire:model="webhookTimeoutSeconds" type="number" min="1" max="120" class="w-full text-sm rounded-xl border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 text-gray-900 dark:text-white focus:ring-purple-600">
            ...
            <div class="sm:col-span-2">
                ...
                <input wire:model="webhookUrl" type="url" class="w-full text-sm rounded-xl border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 text-gray-900 dark:text-white focus:ring-purple-600" placeholder="https://your-api.example.com/skills/inventory-lookup">
            ...
            <div class="sm:col-span-2">
                ...
                <input wire:model="description" type="text" class="w-full text-sm rounded-xl border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 text-gray-900 dark:text-white focus:ring-purple-600" placeholder="What does this skill do?">
            </div>
        </div>
        <div class="flex justify-end gap-2 pt-2">
            ...
            <button wire:click="create" wire:loading.attr="disabled" class="px-4 py-2 bg-purple-600 hover:bg-purple-700 disabled:opacity-75 text-white text-sm font-medium rounded-xl transition-colors">Create</button>
        </div>
    </div>
    @endif
            ...
            @forelse($this->customSkills as $skill)
                <div class="flex items-center justify-between px-4 py-3">
                    ...
                    <div class="flex items-center gap-2 flex-shrink-0">
                        <button wire:click="toggleActive({{ $skill->id }})" class="text-xs font-medium text-purple-600 hover:text-purple-800">
